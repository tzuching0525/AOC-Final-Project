...
def load_test_data():
    # 從文件載入權重
    weight_path = "model_weights/layer_7/mlp_3_weight.txt"
    bias_path = "model_weights/layer_7/mlp_3_bias.txt"
    
    # 載入權重
    with open(weight_path, 'r') as f:
        weights_data = f.read().strip().split()
        weights = torch.tensor([float(x) for x in weights_data]).reshape(64, 128)
...
def load_norm_output():
    ...
    input_path = "my_quantized_outputs/mlp0_quantized_output.txt"
    #input_path = "layer_outputs/transformer_layers_7_mlp_0/quantized_output.txt"
    ...
    with open(input_path, 'r') as f:
        ...
        for line in lines[5:]:  # 跳過前面的統計資訊
            if line.strip():
                values = [max(0, int(x)) for x in line.strip().split()]
...
def test_mlp():
    ...
    # 3. 設置量化參數（從您的模型中獲取）
    #scale = 1/(0.018271807581186295)     1 / ((layer_mlp0.scale = 1) * layer_mlp_3.scale)
    scale = 55
    print(scale)
    # 6 mlp0 = 2041.36
    # 7 mlp0 = 969.566
    # layer_mlp0.scale from cifar_test (x scale)
    # mlp_3.scale from load_fp.py (weight scale)
    zero_point = 0
    ...
    save_quantized_output(quantized_output, 
                         os.path.join(output_dir, "mlp3_quantized_output.txt"))
    
    # 儲存原始輸出
    save_original_output(original_output,
                        os.path.join(output_dir, "mlp3_original_output.txt"))
    
    print(f"\n量化輸出已儲存至 {output_dir}/mlp3_quantized_output.txt")
    print(f"原始輸出已儲存至 {output_dir}/mlp3_original_output.txt")
    
    # 5. 載入實際輸出進行比較
    output_path = "layer_outputs/transformer_layers_7_mlp_3/quantized_output.txt"
