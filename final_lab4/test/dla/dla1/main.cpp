#include <stdio.h>

#include "data.h"
#include "runtime.h"

uint8_t conv_relu_max_ans[16384];

int main() {
    int32_t i;
    int32_t err = 0;

    dla_init();

    printf(
        "[DLA test] qconv2d_relu_maxpool\nInput shape (N=%d, C=%d, H=%d, "
        "W=%d)\nKernel "
        "shape (K=%d, C=%d, H=%d, W=%d)\n\n",
        1, 3, 32, 32, 64, 3, 3, 3);

    /* Run test funcion */
    /*  N -> C -> H -> W

        ofmap_len: 16384 = 1 * 64 * 16 * 16
        ifmap_len: 3072 = 1 * 3 * 32 * 32
        filter_len: 1728 = 64 * 3 * 3 * 3
    */
    qconv2d_relu_maxpool(activation_flat_array, nchw_flat_array,
                         conv_relu_max_ans, bias, (64*16*16), (3*32*32), (64*3*3*3),
                         // mapping parameter
                         64, DEFAULT_e, DEFAULT_p, DEFAULT_q, DEFAULT_r,
                         DEFAULT_t,
                         // shape parameter
                         1, 1, 3, 3, 3, 64, 32, 32,
                         // quantize scale
                         8);

    /* Verify */
    printf("Start checking...\n");
    for (int i = 0; i < 16384; i++) {
        if ((i & 0xff) == 0) {
            printf("[Checking] %d/16384\n", i);
        }
        if (conv_relu_max_ans[i] != conv_relu_max_ans_golden[i]) {
            printf("Index %d is wrong: Correct answer: %d Your answer: %d", i,
                   conv_relu_max_ans_golden[i], conv_relu_max_ans[i]);
            printf("\n");
            err += 1;
        }
    }

    if (err == 0) {
        printf("Simulation all pass!\n");
    } else {
        printf("There are %d errors\n", err);
    }

    dla_final();
    return 0;
}
